...
return {
    ...
    config = function()
        ...
        vim.api.nvim_create_autocmd("LspAttach", {
            ...
            callback = function(event)
                ...
                local function map(mode, lhs, rhs, desc)
                    vim.keymap.set(mode, lhs, rhs, { buffer = event.buf, remap = false, desc = desc })
                end
        ...
        require("mason-lspconfig").setup({
            ensure_installed = {
                "ansiblels", "bashls", "docker_compose_language_service",
                "dockerls", "golangci_lint_ls", "gopls", "lua_ls", "pylsp",
                "rust_analyzer", "terraformls", "ts_ls", "yamlls",
            },
        })
    end,
}
